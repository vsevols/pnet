...
public class StormThrowable extends Exception{
    ...
    public static void WrapExceptions(WrapExceptionVoidCallback callback) throws StormException{
        StormThrowable.WrapExceptions(()->{callback.func();return null;});
    }

    public static void UnsupportedOperation(Object throwContext) {
        if(Debug.throwOnUnsupportedOperation)
            throw new UnsupportedOperationException(new StormThrowable(null, throwContext));

        new StormThrowable(
                new UnsupportedOperationException(new StormThrowable(null, throwContext))
        ).logWarn("");
    }
    ...
    {
        return null != throwable && (throwable.getClass().isInstance(causeClass) || StormThrowable.causeExists(throwable.getCause(), causeClass));
    }
    ...
    {
        return StormThrowable.causeExists(this, causeClass);
    }
